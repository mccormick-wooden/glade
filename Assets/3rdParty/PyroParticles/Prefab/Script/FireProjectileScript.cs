    ...
    public class FireProjectileScript : FireBaseScript, ICollisionHandler
    {
        ...
        private Quaternion FindAppropriateProjectileRotation()
        {
            ...
            while (true)
            {
                ...
                if (++attempts > 100)
                {
                    Debug.Log("Unable to find a suitable beacon landing spot after 100 attempts, firing straight down.");
                 ...
                 */
                if (hit.collider == null || hit.collider.gameObject.CompareTag("TerrainBridge") || IsLandingSpotCloseToBeacon(hit.point))
                {
                    continue;
                }
                ...
                if (hit.collider.TryGetComponent<Terrain>(out Terrain terrain))
                {
                    if (terrain.GetInstanceID() != activeTerrain.GetInstanceID())
                    {
                        ...
                        Debug.LogError("Collided with a terrain that is not the active terrain, this should not happen!");
